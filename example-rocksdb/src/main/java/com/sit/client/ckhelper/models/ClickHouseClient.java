package com.sit.client.ckhelper.models;
...
import com.sit.client.ckhelper.models.utils.POJOMapper;
...
public class ClickHouseClient implements AutoCloseable {
	...
	public ClickHouseClient(String endpoint, String username, String password) {
		...
		AsyncHttpClientConfig config = new DefaultAsyncHttpClientConfig.Builder()
				.setRealm(new Realm.Builder(username, password)
						...
						.build())
				.setConnectTimeout(10 * 1000) // 默认超时时间5000,单位秒(s)
				.build();
	...
	public void close() {
		try {
			...
			LOG.error("Error closing http manager", e);
		}
	}
	...
	public CompletableFuture<String> post(String query) {
		...
		return sendRequest(request).thenApply(res -> "success");
	}
	...
	public CompletableFuture<String> post(String query, List<Object[]> data) {
		...
		// ck那边未返回insert的结果数据
		return sendRequest(request).thenApply(rs -> "success");
	}
