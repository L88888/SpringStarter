package com.sailing.cachemodel.rocksdb;

import com.alibaba.fastjson.JSON;
import com.sailing.cachemodel.RaftUtils;
...
public class DefaultLogEntryImpl implements LogModule {
        ...
        synchronized (this){
            ...
            try {
                ...
                options.setCreateIfMissing(true)
                        .setStatistics(stats)
                        .setWriteBufferSize(1 * SizeUnit.KB)
     ...
     */
    @Override
    public void write(ThirdPartyLog thirdPartyLog,boolean flage) {
        try {
//            lockLog.tryLock(2000, TimeUnit.MILLISECONDS);
            // todo 需要做消息数据值判断，如果为空不能写入
            // 获取最大下标值
            rocksDB.put(thirdPartyLog.getId().getBytes(),
                    JSON.toJSONBytes(thirdPartyLog));
//            rocksDB.put
        } catch (Exception e) {
            e.printStackTrace();
...
//            lockLog.unlock();
    ...
    @Override
    public void write(ThirdPartyLog thirdPartyLog) {
        try {
            lockLog.tryLock(50000, TimeUnit.MILLISECONDS);
            // todo 需要做消息数据值判断，如果为空不能写入
            // 获取最大下标值
            rocksDB.put(thirdPartyLog.getId().getBytes(),
                    JSON.toJSONBytes(thirdPartyLog));
            if (num.incrementAndGet() % 10000 == 0){
                log.warn("写入rockdb成功，写入的数据对象为:>{}", thirdPartyLog);
            }
        } catch (Exception e) {
            e.printStackTrace();
            log.info("写入rockdb数据异常，异常信息为:>{}", e.fillInStackTrace());
    ...
    public ThirdPartyLog read(String key) {
        try {
            byte[] resaultData = rocksDB.get(key.getBytes());
            if (resaultData != null){
