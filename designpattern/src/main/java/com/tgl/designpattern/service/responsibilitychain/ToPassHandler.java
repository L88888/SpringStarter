 ...
 * 第二个处理者
 */
public class ToPassHandler extends AbstractHandler {
    ...
    @Override
    public int handler() {
        System.out.println("第二关卡的业务处理中-->> ToPassHandler");
        int curentScore = getScore();
        if (curentScore >= 90){
            // 表示下一个处理者是否存在，如果存在直接跳转至下一个处理者实例中
            if (!Objects.isNull(this.next)){
                System.out.println("第二关卡业务处理完成,进入下一个关卡业务处理。");
        ...
        System.out.println("第二关卡的业务处理异常。");
        return curentScore;
    }
}
